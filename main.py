...
def read_png(image):
    ...
    with open(image, 'rb') as file:
        ...
        if srgb_chunk:
            ...
            print('Rendering intent:', rendering_intent,
                  {0: 'perceptual', 1: 'relative colorimetric', 2: 'saturation', 3: 'absolute colorimetric'}.get(
                      rendering_intent, 'unknown'))
        else:
        ...
        if phys_chunk and len(phys_chunk[1]) >= 9:
            pixels_per_unit_x, pixels_per_unit_y, unit_specifier = struct.unpack('>IIB', phys_chunk[1][:9])
        ...
        print('Color type:', color_type,
              {0: 'grayscale', 2: 'RGB', 3: 'indexed color', 4: 'grayscale with alpha', 6: 'RGB with alpha'}.get(
                  color_type, 'unknown'))
        print('Compression method:', compression_method,
              {0: 'DEFLATE', 1: 'INFLATE'}.get(compression_method, 'unknown'))
        print('Filter method:', filter_method, {0: 'Adaptive', 1: 'Unknown'}.get(filter_method, 'unknown'))
        print('Interlace method:', interlace_method, {0: 'None', 1: 'Adam7'}.get(interlace_method, 'unknown'))
        #if exif_chunk:
        ...
        if sbit_chunk:
            if color_type in (0, 4):
                ...
                print('Significant bits per channel (RGB):', red_bits, green_bits, blue_bits)
            elif color_type == 3:
                red_bits, green_bits, blue_bits = struct.unpack('>B', sbit_chunk[1][:3])
                alpha_bits = struct.unpack('>B', sbit_chunk[1][3:])[0]
                print('Significant bits per channel (palette):', red_bits, green_bits, blue_bits, alpha_bits)
        ...
        if time_chunk:
            year, month, day, hour, minute, second = struct.unpack('>HBBBBB', time_chunk[1])
            print('Creation time:', f'{year}-{month:02}-{day:02} {hour:02}:{minute:02}:{second:02}')
        else:
        ...
        if chrm_chunk:
            white_point_x, white_point_y, red_x, red_y, green_x, green_y, blue_x, blue_y = struct.unpack('>IIIIIIII',
                                                                                                         chrm_chunk[1])
        ...
        for text_chunk in (chunk for chunk in chunks if chunk[0] == b'tEXt'):
            ...
            try:
                if keyword == b'XML:com.adobe.xmp':
                    ...
                    print('Text, keyword:', keyword.decode(), 'data:', text_data.decode(detected_encoding))
        ...
        for chunk in itext_chunks:
            keyword, compression_flag, compression_method, language_tag, translated_keyword, text = chunk[1].split(
                b'\x00', 5)
